...
{: .note }
Es posible que un usuario pertenezca a grupos multiples!
...
Un ejemplo the usuario regular es `devuser` que puede ser una cuenta local creada for el Administrador de Sistemas.

Un ejemplo de **Super Usuario** es `root` que esta presente en cada sistema de Linux y tiene 100% acceso al sistema entero.

Un ejemplo de **Cuenta de Sistema** es `sshd` que se usa para correr la aplicación de SSH usada para acceso remoto seguro en la red. En cierto modo, una Cuenta de Sistema tiene acceso privilegiado porque toca recursos reservados del sistema. Este tipo de cuenta también se conoce como **Cuenta de Servicio**, la cual no esta asignada a una persona en particular sino que sirve para correr una aplicación o proceso del sistema usada por muchas personas o servicios varios.
...
Los nombres de usuarios tienen ciertas reglas que deberían ser observadas
