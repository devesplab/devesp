...
# Manejo de Paquetes en RedHat con YUM
{: .no_toc }
...
Versiones viejas de RedHat 7 y CentOS 7 usan el comando `yum` para manejar paquetes de software.
...
Cada repositorio en RHEL tiene un fichero correspondiente en `/etc/yum.repos.d/`.
```bash
...
### Encontrar el Paquete Que Ofrece Una utilidad

A veces queremos usar un comando y nos damos cuenta que no esta presente en el sistema. Lo primero que se nos ocurre es instalar la utilidad, pero... cuál es el paquete que nos da la utilidad?

Podemos usar el parámetro `provides` con el comando `yum.`<br>
Para esto usamos esta sintáxis:
```
yum provides <nombre-de-utilidad>
```
...
Para mostrar todos los paquetes relacionados a una categoria usando el parámetro `search` con el comando `yum.`<br>
Para esto usamos esta sintáxis:
```
yum search <nombre-de-utilidad>
```
...
## Bajar e Instalar Paquete de RPM Manualmente
...
yum
: Yellowdog Updater Modified (actualizador y modificador de paquetest de RedHat)
...
Paginas Manuales
- [yum](https://manpages.ubuntu.com/manpages/focal/en/man8/yum.8.html)
